...
function start_process() {
	...
	if [[ ! -d ${path} ]];then
		echo -e "\e[93m目录不存在,新建目录 ${path}\e[0m"
		mkdir -p ${path}
	fi
	cd ${path}
	...
	echo > ${path}/nohup.out
	echo -e "开始启动进程\t${name}"
	echo -e "进程启方式为: nohup ${cmd_array[@]} > nohup.out 2>&1 &"
...
#	fi
	if [[ ${success} -eq 0 ]];then
		echo -e "\e[91m进程\t${name}\t启动失败,开始结束进程...\e[0m"
		stop_process
            echo -e "输出nohup日志文件"
            cd ${path}
            cat nohup.out
